...
DEFAULT_CAMERA_CONFIG = {
    "distance": 3.0,  # 相机到焦点的距离
    "azimuth": 0.0,   # 相机的方位角（水平旋转角度）
    "elevation": -90.0,  # 相机的仰角（垂直旋转角度），设置为 -90 度从上方俯视
    "lookat": np.array([0.8, 0.75, 0.55]),  # 相机的焦点位置
}
...
class MultiMujocoFetchEnv(get_base_fetch_env(MujocoRobotEnv)):
    ...
    def _step_callback(self):
        if self.block_gripper:
            ...
            self._utils.set_joint_qpos(
                self.model, self.data, "robot1:l_gripper_finger_joint", 0.0
            )
            self._utils.set_joint_qpos(
                self.model, self.data, "robot1:r_gripper_finger_joint", 0.0
            )
    ...
    def _set_action(self, action):
        ...
        self._utils.ma_mocap_set_action(self.model, self.data, action)
