...
from gymnasium_robotics.envs.maze.maps import COMBINED, GOAL, RESET, BARRIER, U_MAZE
...
class Maze:
    ...
    * v2 & v1: legacy versions in the [D4RL](https://github.com/Farama-Foundation/D4RL).
    ...
    def barrier_locations(self) -> List[np.ndarray]:
        """Returns all the possible barrier locations in discrete cell
        coordinates (i,j)
        """
        return self._barrier_locations
    ...
    def make_maze(
        ...
        for i in range(maze.map_length):
            for j in range(maze.map_width):
                ...
                if struct == 1:  # Unmovable block.
                    ...
                    maze._barrier_locations.append(np.array([x, y]))
                elif struct == 0:
        ...
        ET.SubElement(
            worldbody,
            "site",
            name="target1",  # 第一个目标的名称
            pos=f"0 0 {maze_height / 2 * maze_size_scaling}",  # 第一个目标的位置
            size=f"{0.5 * maze_size_scaling} {0.5 * maze_size_scaling} {maze_height / 2 * maze_size_scaling}", # 第一个目标的大小
            rgba="1 0 0 0.7",  # 第一个目标的颜色
            type="box"  # 第一个目标的类型
        )
        ...
        ET.SubElement(
            worldbody,
            "site",
            name="target2",  # 第二个目标的名称
            pos=f"1 1 {maze_height / 2 * maze_size_scaling}",  # 第二个目标的位置
            size=f"{0.5 * maze_size_scaling} {0.5 * maze_size_scaling} {maze_height / 2 * maze_size_scaling}",  # 第二个目标的大小
            rgba="1 0 0 0.7",  # 第二个目标的颜色
            type="box"
        )
...
class MultiagentBarrierMazeEnv(GoalEnv):
    def __init__(
        self,
        agent_xml_path: str,
    ...
    def generate_barrier_pos(self) -> np.ndarray:
        assert len(self.maze.barrier_locations()) > 0
        barrier_index = np.random.choice(range(len(self.maze.barrier_locations())),size=2,replace=False)
        barrier = np.array([self.maze.barrier_locations()[barrier_index[index]].copy() for index in range(2)]).reshape(-1)
        return barrier

    def generate_target_goal(self) -> np.ndarray:
        assert len(self.maze.unique_goal_locations) > 0
        #goal_index = self.np_random.integers(
        #    low=0, high=len(self.maze.unique_goal_locations),size=2
        #)
        goal_index = np.random.choice(range(len(self.maze.unique_goal_locations)),size=2,replace=False)
        goal = np.array([self.maze.unique_goal_locations[goal_index[index]].copy() for index in range(2)]).reshape(-1)
        return goal
    ...
    def generate_reset_pos(self) -> np.ndarray:
        ...
        while (
            ...
            #)
            reset_index = np.random.choice(range(len(self.maze.unique_reset_locations)),size=2,replace=False)
            #reset_pos = self.maze.unique_reset_locations[reset_index].copy()
            reset_pos = np.array([self.maze.unique_reset_locations[reset_index[index]].copy() for index in range(2)]).reshape(-1)
    ...
    def reset(
        ...
        *,
        ...
        if options is None:
            ...
            barrier = self.generate_barrier_pos()
        ...
        self.reset_pos = np.array([reset_pos, barrier]).reshape(-1)
