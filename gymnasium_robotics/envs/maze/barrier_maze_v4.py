...
class BarrierMazeEnv(GoalEnv):
    ...
    def generate_target_goal(self) -> np.ndarray:
        ...
        #)
        goal_index = np.random.choice(range(len(self.maze.unique_goal_locations)),size=1,replace=False)
        goal = np.array([self.maze.unique_goal_locations[goal_index[0]].copy()]).reshape(-1)
        return goal
    ...
    def generate_reset_pos(self) -> np.ndarray:
        ...
        while (
            ...
            #)
            reset_index = np.random.choice(range(len(self.maze.unique_reset_locations)),size=1,replace=False)
            #reset_pos = self.maze.unique_reset_locations[reset_index].copy()
            reset_pos = np.array([self.maze.unique_reset_locations[reset_index[0]].copy()]).reshape(-1)
    ...
    def reset(
        ...
        *,
        ...
        self.reset_pos = np.concatenate((reset_pos, barrier))
